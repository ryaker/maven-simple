...
$total_on
$left_on = $total_on - $active_sau

Write-Host "ActiveMin:" $active_min
Write-Host "TotalPost- In:" $left_on

if ($left_on -lt $active_min )
{
    #throw 'Not scaling down because we will go below min'
    Write-Host 'Not scaling down because we will go below min'
    Exit
}
...
$instances | % {
    $StopVMJobs += Stop-AzVmss -ResourceGroupName $vmssrg -VMScaleSetName $vmssname -InstanceId $_ -Force -Verbose -AsJob
}
