#Non-Prod
$nonprod = Select-AzureRmSubscription -SubscriptionName "MTS Development and QA - Non Prod (0602)"
...
{
    $job = Start-AzureRmVM -Name $servers[$i] -ResourceGroupName $rgs[$i] -Verbose -AsJob
}
...
for($i = 0; $i -lt $servers.Count;$i=$i+1)
{
  $IP = (Get-AzureRmPublicIpAddress -Name "$($servers[$i])-ip" -ResourceGroupName $rgs[$i]).IpAddress
  $IPs += $IP
}
...
## Prod
$prod = Select-AzureRmSubscription -SubscriptionName "MTS Development and QA - Prod (0602)"
