...
$scaleOutBlock = get-command "C:\rundeck\scripts\ScaleOutOneSAU.ps1" | select -ExpandProperty ScriptBlock 
$scaleInBlock = get-command "C:\rundeck\scripts\ScaleInOneSAU.ps1" | select -ExpandProperty ScriptBlock 

do {
    $scaleSetList = "zus2pwssr" #"zus2pwssg", "zus2pwssb", "zuswpwssg", "zuswpwssb", "zseapwsg2", "zseapwsb2", "zeuwpwsg2", "zeuwpwsb2"
    ...
    foreach ( $scaleSet in $scaleSetList )
    {
        ...
        if ( $disposition -eq [ScaleDisposition]::ScaleOut ) {
            Invoke-Command -ScriptBlock $scaleOutBlock -ArgumentList $scaleSet
