...
for ( $i=0; $i -lt $vmss.Sku.Capacity; $i++ ) {  
    ...
    $status = (Get-AzureRmVmssVM -InstanceView  -ResourceGroupName $rgName -name $vmssName  -InstanceId $i).Statuses.DisplayStatus[1]
...
$ids = $($Count-4)..$Count-1
$ids | % {
Get-AzureRmVMssVM -ResourceGroupName $vmssrg -VMScaleSetName $vmssname -InstanceId $_
#Start-AzureRmVmss -ResourceGroupName $vmssrg -VMScaleSetName $vmssname -InstanceId $_ -Verbose 
}
}
sleep 60
