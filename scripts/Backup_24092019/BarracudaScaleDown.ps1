...
if($ServerType -eq "VMSS")
{
    ...
    $servers = $vm
    Write-Host "Half of servers will be stopped from $servers"

    $ids = $(($servers.Count)/2)..$(($servers.Count)-1)
    $ids | % {
    Stop-AzureRmVmss -ResourceGroupName $rg -VMScaleSetName $vmssname -InstanceId $InstanceId[$_] -Force -Verbose -AsJob
...
if($ServerType -eq "VM")
{
    ...
    Stop-AzureRmVM -Name $_ -ResourceGroupName $rg -Force -Verbose -AsJob
    }    
}
...
write-host "Scale down completed on $BarracudaDC"