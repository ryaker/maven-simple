...
<#
Get-AzureRmRedisCache | Select -Property Name,ResourceGroupName
#>
...
Set-AzureRmRedisCache -ResourceGroupName $Redisrg -Name $RedisServiceName -ShardCount 4

sleep 120

$redis = Get-AzureRmRedisCache -ResourceGroupName $Redisrg -Name $RedisServiceName
...
{
    Write-Host "$($redis.Name) is scale down activity completed"
}