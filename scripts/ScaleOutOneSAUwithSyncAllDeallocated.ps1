...
switch ($vmssname) {
    "zuswpwssb" { $vmssrg = "zusw-prod-web-rg"; $role = "web"; $bad_instances =@(8,24,34); $source = "zuswpwssb000000"}
    "zuswpwssg" { $vmssrg = "zusw-prod-web-rg"; $role = "web"; $bad_instances =@(28,33) ; $source = "zuswpwssg000000"}
    "zus2pwssb" { $vmssrg = "zusw2-prod-web-vmss-rg"; $role = "web"; $bad_instances =@(); $source = "zus2pwssb000000"}
    "zus2pwssg" { $vmssrg = "zusw2-prod-web-vmss-rg"; $role = "web"; $bad_instances =@(9); $source = "zus2pwssg000000" }
    ...
    "zus2psssb" { $vmssrg = "zusw2-prod-svc-rg"; $role = "svc"; $bad_instances =@(16); $source = "zus2psssb000000" }
...
for ( $i = 0; $i -lt $vmss.Sku.Capacity; $i++ ) {
    ...
    if (($status -ne "VM running") -and ($all_instances[$i] -notin $bad_instances)) {
        $servers += $servername
...
#region Sync EventSource
$syncEventSourceJobs = @()
$sourceApplications = Invoke-Command -ComputerName $sourceServer -ScriptBlock {
    $applications = (Get-ChildItem -Path "HKLM:\System\CurrentControlSet\services\eventlog\Application\").Name
    $applicationLeafs = Split-Path $applications -leaf
    
    $applicationLeafs
}

$servers | ForEach-Object {
    $syncEventSourceJobs += Invoke-Command -ComputerName "$_.hrbl.net" -ScriptBlock {
        param([array]$sourceApplications)

        for($x=0; $x -lt $sourceApplications.Length; $x++) {
            $appName = $sourceApplications[$x]
        
            if (-not (Test-Path "HKLM:\System\CurrentControlSet\services\eventlog\Application\${appName}")) {
                New-Item -Path "HKLM:\SYSTEM\CurrentControlSet\services\eventlog\Application\" -Name "${appName}" -Force
                Write-Host "Event source : ${appName} created on $env:COMPUTERNAME"
            }
        }
    } -ArgumentList (,$sourceApplications) -Verbose -AsJob
}

$syncEventSourceJobs | ForEach-Object {
    Wait-Job $_
    $receivedJob = Receive-Job $_

    Write-Host $receivedJob
}
#endregion
