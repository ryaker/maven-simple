...
for ( $i = 0; $i -lt $vmss.Sku.Capacity; $i++ ) { 
    ...
    $vmstatus = (Get-AzVMssVM -InstanceView  -ResourceGroupName $vmssrg -VMScaleSetName $vmssname  -InstanceId $i).Statuses.DisplayStatus[1]
    Write-Host "$($vm.OSProfile.ComputerName) : $vmstatus"
